...
def update_vdi_list(self , var):
    try:
        ...
        if var.VDI_CHANGED_FILEDATA:
            ...
            for vdi_index in var.VDI_CHANGED_FILEDATA:
                count_column : int  = 0  # 初始化列数索引
                for vdi_item in vdi_index.keys():
                    if vdi_item == 'path':
                        temp_item : QTableWidgetItem = QTableWidgetItem(os.path.join(var.VDI_BASEPATH , *vdi_index[vdi_item]))
                    elif vdi_item == 'type':
                        temp_item : QTableWidgetItem = QTableWidgetItem(vdi_index[vdi_item])
                        # 根据类型设置背景颜色
                        if vdi_index[vdi_item] == 'folder':
                            temp_item.setBackground(QColor(0 , 255 , 0 , 32))
                            
                        elif vdi_index[vdi_item] == 'file':
